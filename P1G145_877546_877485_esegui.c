...
/*
 Nota KB
 L'area di memoria occupata dallo stack ha creato abbastanza confusione: confrontandosi con altri colleghi, abbiamo trovato molte versioni discordanti cu come creare lo stack della macchina virtuale.
 Dopo aver valutato le varie opinioni, abbiamo deciso di attenerci alla nostra versione:
 1 - Il simbolo esatto è "kB" con la k minuscola... altre simbologie come KB, K... sono molto diffuse ma formalmente scorrette
 2 - 1kB = 1000 bytes, non 1024
 3 - 1024 bytes = 1 kibibyte (simbolo KiB)

 Fonte:
 Sito BPIM (Bureau International del Poids et Mesures)
 https://www.bipm.org/en/about-us/
 Link diretto alla brochure in inglese:
 https://www.bipm.org/utils/common/pdf/si-brochure/SI-Brochure-9-EN.pdf
 Capitolo 3, pagina 143: il Sistema Internazionale definisce il prefisso "k" come multiplo usato per una potenze di 10 (nello specifico, 10 alla terza). A lato pagina, c'è una nota apposita per i prefissi usati con le potenze di 2.

 Moodle
 Sul forum, nello "Spazio per le internazioni tra studenti e tra studenti e tutor" è stata posta la domanda:
 "Lo stack della Virtual Machine è uno solo e a grandezza fissa (64kb = 16000 celle?)"
 Risposta: Si
 Perché lo stack sia composto da 16.000 elementi, si deve avere 64kb = 64.000 bytes.
 */
...
int esegui(const char *input)
{
    ...
    while ((errori == 0) && ((++ip) < numero_istruzioni) && (programma[ip] != 0))
    {
        ...
        {            /* n numeri dello stack da stampare */
